...
export function LinkList({ className, links }: Props) {
  return (
    <div className={clsx(className, 'flex flex-col items-center justify-center text-center')}>
      {links.map(
        (
          link: { text: string; link: { href: string; target?: '_self' | '_blank' } },
          index: number
        ) => (
          <Link
            key={index}
          ...
          >
            ...
            <div className="grid size-8 -translate-x-4 place-items-center bg-primary opacity-0 transition-all duration-300 ease-out group-hover/link:translate-x-0 group-hover/link:opacity-100 sm:-translate-x-6 md:size-10 md:translate-y-0.5 xl:size-12 xl:translate-y-1">
